...
@WORKFLOWS.register_module("R3L_dapo_w_o_credit_workflow")
class R3LDapoWoCreditWorkflow(Workflow):
    """
    R3L workflow for DAPO mathematical problem solving without credit assignment (ablation study)
    ...
    def __init__(
        ...
        print(
            f"Initializing R3LDapoWoCreditWorkflow (w/o credit assignment), temperature={self.temperature}"
        )
        self.reset(task)
    ...
    def get_reflect(
        ...
        try:
            ...
            print(f"[R3L w/o credit] Reflection failed - Error: {str(e)}")
            return None, None, None

    def run(self) -> List[Experience]:
        """Run the R3L dapo workflow and return experiences (without credit assignment)"""
        ...
        for i in range(self.n // 2):  # Half for rollout, half for reflection + retry
            try:
                ...
                print(f"[R3L w/o credit] First rollout - reward: {reward}, attempts: {attempts}")
                ...
                print(f"[R3L w/o credit] Starting reflection on first attempt (reward: {reward})...")
                ...
                if reflect_checklist is None:
                    print(f"[R3L w/o credit] Reflection failed - No valid reflection data generated")
                elif is_valid and not is_perfect:
                    print(f"[R3L w/o credit] Reflection successful - Valid reflection generated")
                elif is_perfect:
                    print(f"[R3L w/o credit] Reflection indicates perfect first attempt - No retry needed")
                elif not is_valid:
                    print(f"[R3L w/o credit] Reflection validation failed - Invalid reflection data")
                ...
                if not is_valid or is_perfect:
                    ...
                    print("[R3L w/o credit] Valid reflection obtained, proceeding to second rollout...")
                    ...
                    try:
                        ...
                        print(
                            f"[R3L w/o credit] Second rollout - reward: {second_reward}, attempts: {second_attempts}, improve: {second_reward > reward}"
                        )
                        ...
                        if second_reward > reward and second_reward >= 1.0:
                            print(
                                f"[R3L w/o credit] Second attempt successful improvement - Recording reflection and retry experiences"
                            )
                            print(
                                f"[R3L w/o credit] Reward improvement: {reward} -> {second_reward} (+{second_reward - reward:.2f})"
                            )
                            reflect_exp.reward = 1.0
                            # Set eid
                            reflect_exp.eid.task = str(self.task.task_id) + f"_reflect_{i}"
                            ...
                            print(
                                "[R3L w/o credit] Reflection and retry led to improvement, recording both..."
                            )
                        elif second_reward <= reward:
                            print(
                                f"[R3L w/o credit] Second attempt did not improve - First reward: {reward}, Second reward: {second_reward}"
                            )
                        else:
                            print(
                                f"[R3L w/o credit] Second attempt improved but below threshold - Reward: {second_reward} (need >= 1.0)"
                            )
                    except Exception as e:
                        print(f"[R3L w/o credit] Second rollout failed - Error: {str(e)}")
            except Exception as e:
                print(f"[R3L w/o credit] Rollout iteration {i} failed - Error: {str(e)}")

        # Print summary statistics
        print(f"\n[R3L w/o credit Summary] Generated {len(exp_lst)} experiences")
        total_reward = sum(exp.reward for exp in exp_lst)
        avg_reward = total_reward / len(exp_lst) if exp_lst else 0.0
        print(f"[R3L w/o credit Summary] Total reward: {total_reward:.2f}, Average reward: {avg_reward:.2f}")
