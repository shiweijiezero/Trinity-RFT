...
Group-Relative Policy Optimization (GRPO) \cite{shao2024deepseekmath} has emerged as a powerful critic-free RL technique for large language models, powering systems like DeepSeek-R1 \cite{guo2025deepseek} and DeepSeek-Math \cite{shao2024deepseekmath}. Despite these successes, GRPO struggles on complex multi-step tasks due to three fundamental limitations:

\begin{itemize}
    \item \textbf{Inefficient exploration.} Stochastic sampling generates predominantly failed trajectories ($>$90\% on challenging tasks). When all group samples fail, reward variance becomes zero, producing null gradients \cite{nan2025ngrpo}. Scalar rewards indicate correctness but provide no actionable guidance for improvement \cite{zhang2025critique}.
    \item \textbf{Coarse credit assignment.} Trajectory-level rewards penalize entire sequences uniformly. A solution with correct steps 1-5 but wrong step 6 receives negative gradients for all steps, suppressing valid reasoning.
    \item \textbf{Training instability.} Failed trajectories ($\sim$70\%) numerically dominate batches even with guided exploration. Negative signals suppress incorrect actions but lack positive guidance, destabilizing training through overwhelming suppression in unpredictable directions \cite{wu2025learning}.
\end{itemize}
...
\textbf{Agentic Environments.} Performance gains scale with task complexity. On ALFWorld (relatively structured), R$^3$L achieves 0.928 success rate, an 11\% improvement over OPMD (0.835). The moderate gain reflects ALFWorld's clear action space where even stochastic sampling occasionally succeeds (RAFT: 0.826).

On WebShop (semi-structured e-commerce), R$^3$L reaches 0.365, a 26\% relative improvement over GRPO (0.29). Here, the search space expands dramatically: agents must navigate product catalogs, compare attributes, and make purchasing decisions. GRPO's stochastic sampling generates $\sim$70\% failed trajectories; our reflect-then-retry increases successful samples to $\sim$35\%, directly explaining the performance gap.

The advantage becomes dramatic on ScienceWorld (unstructured scientific reasoning). R$^3$L achieves 0.122 success rate, 2.5$\times$ higher than GRPO (0.049) and 7.6$\times$ higher than OPMD (0.016). This task requires formulating hypotheses, conducting experiments, and drawing conclusions over 15-20 steps. GRPO's success rate is $<5\%$—effectively random exploration. Language feedback in R$^3$L identifies specific experimental flaws (e.g., "forgot to heat the solution before testing pH"), enabling targeted corrections impossible through stochastic resampling alone.

\textbf{Mathematical Reasoning.} Gains are even more pronounced here due to mathematical reasoning's binary correctness: a single algebraic error invalidates entire solutions, making credit assignment critical. On GSM8K, R$^3$L achieves 0.721 accuracy, a 24.7-point absolute improvement over GRPO (0.474). Analysis reveals GRPO solutions often contain correct initial problem decomposition (steps 1-5) followed by calculation errors (step 6). Trajectory-level rewards suppress all steps; Pivotal Credit Assignment preserves the valid prefix, accelerating learning.

This pattern holds across benchmarks with varying difficulty. Math500 (diverse problem types) improves by 13.5 points (0.439 vs. 0.304). Competition-level benchmarks show substantial gains: Olympiad +7.8 points (0.168 vs. 0.090), AMC23 +5.0 points (0.250 vs. 0.200), though smaller absolute values reflect these datasets' inherent difficulty. On MinervaMath (advanced calculus/algebra), R$^3$L (0.120) achieves 21\% relative improvement over GRPO (0.099) despite $<$13\% base success rate. DAPO-Test shows +2.0 points (0.156 vs. 0.136), the smallest gain, attributable to DAPO's training set already containing diverse augmentations that partially overlap with our reflect-then-retry generated data.
...
\textbf{Positive Preference Optimization (PPO).} Removing PPO causes the most severe degradation: GSM8K drops 15.3 points (0.721 → 0.568), WebShop 7.0 points (0.365 → 0.295), ScienceWorld 4.9 points (0.122 → 0.073). The mechanism is gradient imbalance. Even with retry generating 30\% successful samples, failed trajectories (70\%) numerically dominate the batch. Standard advantage normalization gives equal weight to success and failure signals. Without PPO's $\alpha=3.0$ amplification, the learning signal becomes: $\mathbb{E}[\nabla] \approx 0.3 \nabla_{success} + 0.7 \nabla_{failure}$, where negative signals swamp positive guidance. Training diverges on ScienceWorld (4.9\% → 7.3\% degradation) where even retry succeeds $<$20\% of the time.

\textbf{Pivotal Credit Assignment (PCA).} Removing PCA causes 10.9-point drop on GSM8K (0.721 → 0.612), 5.0 points on WebShop (0.365 → 0.315). Without pivotal masking, gradient updates apply to entire trajectories. Consider a GSM8K solution: "Step 1-5: correct problem decomposition (5 tokens) → Step 6: $3 \times 4 = 15$ [error] (1 token)." Trajectory reward $R=0$ causes $\nabla\ell \propto -\log\pi$ for all 6 steps, suppressing valid steps 1-5. PCA masks steps 1-5, applying updates only to step 6, preserving sound reasoning. Impact is larger on math (10.9 vs. 5.0 points) because math has longer valid prefixes before errors.

\textbf{Reflect-then-Retry (RR).} Removing RR reduces GSM8K by 12.7 points (0.721 → 0.594), WebShop by 6.0 points (0.365 → 0.305). Without reflection, the model becomes GRPO with PCA+PPO applied to stochastically sampled trajectories. On GSM8K, base sampling success rate is 9\%; retry increases this to 35\%. Removing retry eliminates 26\% of high-quality data, directly explaining the 12.7-point drop. Interestingly, w/o RR still outperforms GRPO baseline (0.594 vs. 0.474) because PCA+PPO effectively learn from the sparse 9\% successes—validating that each component contributes independently.
...
R$^3$L has several limitations. First, it incurs 35\% computational overhead compared to GRPO due to reflection generation. While this is offset by 2-3$\times$ sample efficiency on complex tasks, simple tasks may not benefit proportionally. Second, performance depends on reflection quality, which varies with prompt design and model capability. Smaller models ($<$1B parameters) may struggle to generate actionable feedback. Third, R$^3$L requires tasks with verifiable rewards for retry evaluation. Tasks with subjective or ambiguous success criteria (e.g., open-ended creative writing) cannot leverage our framework without human-in-the-loop verification. Finally, while R$^3$L improves learning efficiency, it does not address potential biases in the base language model, which may be reinforced during training.

\section*{Ethics Statement}

This work aims to improve language model training efficiency and reliability through more effective reinforcement learning. Potential positive impacts include reducing computational waste and improving model performance on reasoning tasks critical for scientific and educational applications. However, we acknowledge dual-use concerns: improved RL methods could be misused to train models for generating harmful content or misinformation at scale. We strongly advocate for responsible deployment with appropriate safeguards. All experiments use publicly available datasets with permissive licenses (ALFWorld: MIT, WebShop: Apache 2.0, ScienceWorld: Apache 2.0, DAPO: CC-BY-4.0, GSM8K: MIT). Our training infrastructure (8×H800 GPUs for 3 epochs) has environmental impact; we estimate 450 kWh total energy consumption. Future work should explore carbon-efficient training strategies.
